...
| 层 | 内容 |
|---|---|
| 编排层 | 画布编辑器、节点配置面板、调试运行面板、撤销/重做/复制 |
| 执行层 | DAG 调度引擎、节点运行时、上下文与状态机 |
| 数据层 | 工作流定义、执行记录与日志、模型密钥配置 |
...
系统**默认使用 `MockLLMProvider`**（离线、不花钱、不依赖密钥），适合本地开发与演示。要跑出**真实智能结果**（尤其是教学节点的出题/批改/学情），需切换到真实模型。
...
- [x] 用户操作说明书（功能、架构、操作步骤、技术特点、常见问题）
